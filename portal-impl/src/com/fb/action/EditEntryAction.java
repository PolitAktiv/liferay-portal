 ...
 */
public class EditEntryAction extends
		com.liferay.portlet.blogs.action.EditEntryAction {
	...
	public ActionForward render(ActionMapping actionMapping,
		...
		ActionForward forward = super.render(actionMapping, actionForm,
				portletConfig, renderRequest, renderResponse);

		BlogsEntry entry = (BlogsEntry) renderRequest
				.getAttribute("BLOGS_ENTRY");
	...
	protected void addOpenGraphProperties(PortletRequest request,
		...
		PortletURL portletURL = ((LiferayPortletResponse) response)
				.createRenderURL();

		portletURL.setParameter("struts_action", "/blogs/view_entry");
		portletURL.setParameter("urlTitle", entry.getUrlTitle());

		String url = PortalUtil.getCanonicalURL(portletURL.toString(),
		 ...
		 * String twSite = "";
		 * 
		 * if (Validator.isNotNull(twSite)) { twitterAttributes.put("site",
		 * StringPool.AT + twSite); }
		 */
		...
		if (Validator.isNotNull(contact.getTwitterSn())) {
			twitterAttributes.put("creator",
					StringPool.AT + contact.getTwitterSn());
		}
